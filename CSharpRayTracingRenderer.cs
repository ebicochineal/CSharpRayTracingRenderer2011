...
class BoundingBox
{
    ...
    public static bool RayAABB(Ray ray, BoundingBox BBox)
    {
        ...
        {
            if (ray.Position.x >= BBox.min_x && ray.Position.x <= BBox.max_x && ray.Position.y >= BBox.min_y && ray.Position.y <= BBox.max_y)
            { xy = true; }
        }
        if (ray.Direction.z == 0f && ray.Direction.x == 0f)
        {
            if (ray.Position.z >= BBox.min_z && ray.Position.z <= BBox.max_z && ray.Position.x >= BBox.min_x && ray.Position.x <= BBox.max_x)
            { zx = true; }
        }
        if (ray.Direction.y == 0f && ray.Direction.z == 0f)
        {
            if (ray.Position.y >= BBox.min_y && ray.Position.y <= BBox.max_y && ray.Position.z >= BBox.min_z && ray.Position.z <= BBox.max_z)
            { yz = true; }
        }
